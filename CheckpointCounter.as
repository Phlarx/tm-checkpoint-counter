...
string getDisplayText() {
	...
	case EDispMode::ShowRemainingAndTotal:
		return "-" + (maxCP - curCP) + " / " + maxCP;
	case EDispMode::ShowCompletedAndRemaining:
		return curCP + " / -" + (maxCP - curCP);
...
void Render() {
	if(showCounter && inGame) {
		...
		if(showBackground) {
			...
			vec2 size = nvg::TextBoxBounds(anchorX * Draw::GetWidth() - 100, anchorY * Draw::GetHeight(), 200, getDisplayText());
			nvg::BeginPath();
		...
		nvg::TextBox(anchorX * Draw::GetWidth() - 100, anchorY * Draw::GetHeight(), 200, getDisplayText());
	}
}
